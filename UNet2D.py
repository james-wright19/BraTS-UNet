...
class DownBlock2D(torch.nn.Module):
    def __init__(self, in_channels, out_channels, padding=False):
        ...
        if padding:
            self.conv = ConvBlock2D(in_channels, out_channels, 1)
        else: 
            self.conv = ConvBlock2D(in_channels, out_channels, 0)
...
class UpBlock2D(torch.nn.Module):
    def __init__(self, in_channels, out_channels, padding=False):
        ...
        if padding:
            self.conv = ConvBlock2D(in_channels+out_channels, out_channels, 1)
        else:
            self.conv = ConvBlock2D(in_channels+out_channels, out_channels, 0)
